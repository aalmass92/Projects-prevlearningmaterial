/* The Server. Shared memory segments synchronyzed with semaphores. This program uses shared memory with key 99 and semaphore set with key 9900 */
#include <stdio.h>
#include <sys/shm.h>
#include <time.h>
#include <sys/sem.h>
...
if ( mem_ptr == (void *)- 1 )
oops("shmat",2);
/* create a semset: key 9900, 2 semaphores, mode rw-rw-rw */
...
/* build and execute a 2 - element action set: wait for 0 on n_readers and increment n_writers*/
wait_and_lock(int semset_id)
{
	...
	actions[1].sem_op = +1; /* increment num writers */
	if ( semop( semset_id, actions, 2) == -1)
		oops("semop:locking", 10);
}
/* build and execute a 1-element action set. Decrement num_writers */
